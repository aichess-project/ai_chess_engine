...
#
# The Random Array is copied from https://github.com/niklasf/python-chess/blob/master/chess/polyglot.py
# 
...
class ZobristHash:
    ...
    def __init__(self, random_array = None):
        if random_array is None:
          random_array = POLYGLOT_RANDOM_ARRAY
        self.random_array = random_array

    def get_square_piece_index(self, square_index, piece_index):
      return square_index + piece_index * 64

    def get_square_piece_indexvalue(self, square_index, piece_index):
      return self.random_array[self.get_square_piece_index(square_index, piece_index)]

    def update_hash_piece_square(self, square_index, piece_index):
      random_value = self.get_square_piece_indexvalue(square_index, piece_index)
      self.hashValue ^= random_value

    def update_hash_rest(self, ep, turn, wck, wcq, bck, bcq):
      if turn:
        self.hashValue ^= self.random_array[ZobristHash.TURN_INDEX]
      if wck:
        self.hashValue ^= self.random_array[ZobristHash.WCK_INDEX]
      if wcq:
        self.hashValue ^= self.random_array[ZobristHash.WCQ_INDEX]
      if bck:
        self.hashValue ^= self.random_array[ZobristHash.BCK_INDEX]
      if bcq:
        self.hashValue ^= self.random_array[ZobristHash.BCQ_INDEX]
      if ep:
        if turn == chess.BLACK:
          # a3 is Square 16
          index = ep-16
          ...
          # a6 is Square 40
          index = ep-40
        self.hashValue ^= self.random_array[ZobristHash.EP_INDEX + index]

    def update_hash_pos_new(self):
       for pivot, squares in enumerate(self.board.occupied_co):
            for square in chess.scan_reversed(squares):
                piece_index = (typing.cast(chess.PieceType, self.board.piece_type_at(square)) - 1) * 2 + pivot
                self.hashValue ^= chess.polyglot.POLYGLOT_RANDOM_ARRAY[64 * piece_index + square]
       
    def update_hash_pos_old(self):
        for square_index, square in enumerate(chess.SQUARES_180):
            piece = self.board.piece_at(square)
            if piece:
                self.update_hash_piece_square(square_index, piece.__hash__())

    def set_board(self, board, variant = 1):
        self.hashValue = 0
        self.board = board
        if variant == 1:
            self.update_hash_pos_old()
        else:
            self.update_hash_pos_new()

        self.update_hash_rest(board.ep_square, board.turn,
